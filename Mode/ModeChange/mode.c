...
/**
  * @brief  Update active mode - dispatch to current mode's update function
  * @note   Call this in main loop to update the active mode
  * @retval None
  */
void Mode_UpdateCurrent(void)
{
    switch (g_current_mode) {
        ...
        case MODE_CURVE:
            Curve_Update();
            break;
        case MODE_LINEFOLLOWSEQUENCE:
            LFS_Update();
            break;
        default:
            /* Unknown mode - fallback to triangle */
            Triangle_Update();
            break;
    }
}
